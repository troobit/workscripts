...
- **Machine-specific checks are backup-aware**: `verify-setup.sh`'s PRISMPATH/cppr "kept local" checks (`check_kept_local`) only fail when a pre-migration backup under `~/.workscripts-backups/.zshrc.*` contains the setting but the live `~/.zshrc` doesn't. On a fresh machine they report skipped, keeping Req 3.7 (fresh and drifted machines converge to the same verified state) intact.
- **`--local` collects no identity**: `new-mac.sh --local` skips the interactive phase, so `GIT_NAME`/`GITHUB_EMAIL` are unset; the gitconfig section must stay guarded on those vars or it writes a `~/.gitconfig` with empty name/email (there is no `set -u` in new-mac.sh to catch it).
